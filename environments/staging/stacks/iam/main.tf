module "iam" {
  source     = "git::https://github.com/temitayocharles/terraform-module.git//module/iam?ref=main"
  iam_config = local.env.iam_config
}

module "oidc" {
  count  = try(local.env.oidc_config.enable_github_oidc, false) ? 1 : 0
  source = "git::https://github.com/temitayocharles/terraform-module.git//module/oidc?ref=main"
  oidc_config = {
